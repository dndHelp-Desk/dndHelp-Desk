...
const Chat:FC = () => {
  ...
  const thread =
    ...
    threadMessage.map((message, index) => {
      return (
        <div
          key={index}
          className="w-full snap_child text-slate-400 text-sm leading-6 p-2 rounded flex overflow-hidden gap-2"
        >
          <div
            className={`h-[2rem] w-[2rem] min-w-[2rem] hidden sm:flex justify-center items-center rounded uppercase text-lg text-gray-300 bg-slate-800 ${
          ...
          <div className={`w-[95%] 2xl:w-full  bg-slate-800 p-4 rounded`}>
  ...
  //Component ===============================
  return (
    <div className=" bg-slate-600 custom-shadow h-[35rem]  max-w-[45rem] w-full rounded mt-4 p-2 flex flex-col">
      ...
      <div className="h-[45%] lg:h-[40%] w-full bg-transparent p-4 pt-6 flex items-center justify-center relative">
        <div className="h-full w-full p-2 rounded  bg-slate-800 after:content-[''] after:absolute after:top-[1rem] after:left-[5rem] after:mt-[-15px] after:border-[12px] after:border-t-transparent after:border-r-transparent dark:after:border-b-slate-800 after:border-b-slate-800 after:border-l-transparent overflow-hidden grid grid-rows-4 gap-2">
          <div className="w-full row-span-1 h-full flex justify-between">
            ...
            <button className="h-9 w-9 rounded dark:hover:bg-slate-900 hover:bg-slate-300 flex items-center justify-center text-slate-500 transition-all outline-none focus:outline-none text-base font-bold">
              <BsThreeDotsVertical />
            </button>
          </div>
          <form
            onSubmit={(e) => sendReply(e)}
            className="row-span-3 h-full w-full bg-transparent rounded relative"
          >
            <textarea
              name="reply"
              id="reply"
              ...
              className="h-full w-full  bg-slate-800 rounded resize-none text-sm dark:text-slate-400 text-slate-500 border-0 focus:outline-none outline-none focus:border-0 transition-all dark:focus:ring-slate-700 focus:ring-slate-700 placeholder:text-slate-500 placeholder:text-sm"
            ></textarea>
            <button
              type="submit"
              className="absolute outline-none focus:outline-none focus:ring-1 focus:ring-blue-600 bottom-2 rounded text-sm right-2 p-2 px-4 font-semibold  text-slate-100 bg-blue-700 z-[99]"
            >
